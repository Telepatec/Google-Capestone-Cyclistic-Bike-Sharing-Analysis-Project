...
SELECT
  membership_type,
  part_of_day,
  COUNT(part_of_day) AS rides_per_time_zone,
  SUM(COUNT(part_of_day)) OVER (PARTITION BY time_zone) AS total_rides_per_time_zone,
  CONCAT(CAST(COUNT(part_of_day) * 100.0 / SUM(COUNT(part_of_day)) OVER (PARTITION BY time_zone) AS DECIMAL(10,2)), '%') AS percentage_of_rides
FROM (
   SELECT
     member_casual AS membership_type,
